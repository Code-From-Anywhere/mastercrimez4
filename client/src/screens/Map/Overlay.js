...
const Overlay = ({
  ...
  dragAndDropMode,
  setDragAndDropMode,
  territoriesViewPagerRefContainer,
      ...
      return [
        {
          ...
          onPress: async () => {
            ...
            getAreas();
            reloadMe(device.loginToken);
            alertAlert(response, null, null, { key: "takeEmptyAreaResponse" });
          },
          icon: Icon.MaterialCommunityIcons,
          iconName: "account-arrow-left",
        },
  ...
  const renderTerritoriesViewPager = (
    <ViewPager
      key={`viewPager1`}
      ref={territoriesViewPagerRefContainer}
    ...
    >
      ...
      {cityAreas.areas.map((area, index) => {
        ...
        return (
          <View key={`page${index}`} style={{ height: 150 }}>
            <Grid>
              <Col>
  ...
  const renderPropertiesViewPager = (
    <ViewPager
      key={`viewPager2`}
      ref={propertiesViewPagerRefContainer}
  ...
  const title = selected === "area" ? selectedArea?.name : currentObject?.title;
  ...
  return (
    <>
      ...
      >
        <SafeAreaView>
          {view === "territories" ? (
            renderTerritoriesViewPager
          ) : view === "crimes" ? (
            <Stats device={device} me={me} />
          ) : (
            renderPropertiesViewPager
          )}
        </SafeAreaView>
      </View>
